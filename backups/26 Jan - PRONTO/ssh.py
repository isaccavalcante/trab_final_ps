...
class JanelaSSH:
    ...
    def logar(self):
        self.janela.set_title("share - ssh %s@%s" % (self.login, self.ip))
        ...
        self.botao_executar = gtk.Button("Executar Comando")
        self.botao_executar.connect("clicked", self.executar)

        self.botao_voltar = gtk.Button("<Voltar")
        self.botao_voltar.connect("clicked", self.voltar)

        self.botao_logout = gtk.Button("Logout")
        self.botao_logout.connect("clicked", self.logout)
	...
	'''
        ...
        #self.caixa2.pack_star(self.caixa2)
        #self.caixa2.pack_start(self.label_comando)
        #self.caixa2.pack_start(self.texto_comando)
        #self.caixa2.pack_start(self.botao_executar)
        ...
        self.caixa3.pack_start(self.fixo2)
        #self.caixa3.pack_start(self.label_info)
	...
	#######
	#######
        #vbox.pack_start(self.label_info, False, False, 0)
        #self.label_info.show()

	#vbox.pack_start(self.label_comando, False, False, 0)
        #self.label_comando.show()
	...
	###
	#fixo2 = gtk.Fixed()
	#fixo2.put(self.label_comando, 30, 420)
	#self.label_comando.show()
	#box1.pack_start(fixo2, True, True, 0)
	###
	vbox.pack_start(self.texto_comando, False, True, 0)
	self.texto_comando.show()
        vbox.pack_start(self.botao_executar, True, True, 0)
	self.botao_executar.show()
	vbox.pack_start(self.botao_logout, True, True, 0)
	self.botao_logout.show()

	self.janela.add(self.box1)
        #self.janela.add(self.caixa3)
        #self.janela.show_all()

    def logout(self, widget, data=None):
        self.janela.set_title('share - ssh')
        self.janela.remove(self.box1)
        self.janela.add(self.fixo)
